...
class MySequential(agent_components.action_spec_ignored.ActionSpecIgnored):
  def __init__(
        self,
        model: language_model.LanguageModel,
        question_after_summarization: str,
        components_order: list[str],
        # component_to_run_first: str,
        # component_to_run_second: str,
        pre_act_key: str = 'MySequential',
    ):
    super().__init__(pre_act_key)
    self._model = model
    self._question_after_summarization = question_after_summarization
    self._components_order = components_order

  def _make_pre_act_value(self) -> str:
    prompt = interactive_document.InteractiveDocument(self._model)

    for component in self._components_order:
      component_pre_act_value = self.get_named_component_pre_act_value(component)
      prompt.statement(f'{component}: {component_pre_act_value}')

    result = prompt.open_question(
        question=self._question_after_summarization
    )
    return result

class ResponsibilityQuestion(agent_components.question_of_recent_memories.QuestionOfRecentMemories):
  def __init__(
      self,
      agent_name:str,
      **kwargs,
...
class TradeoffQuestion(agent_components.question_of_recent_memories.QuestionOfRecentMemories):
  def __init__(
      self,
      agent_name:str,
      **kwargs,
...
class MoralQuestion(agent_components.question_of_recent_memories.QuestionOfRecentMemories):
  def __init__(
      self,
      agent_name:str,
      **kwargs,
...
def _make_about_component(
  ...
  sequence_question = MySequential(
    model, 
    question_after_summarization=f'Given the information above, how should {agent_name} behave to make everybody feel better?',
    components_order=[
  ...
  return (responsibility_question, tradeoff_question, moral_question, sequence_question)

class ActionQuestion(agent_components.question_of_recent_memories.QuestionOfRecentMemories):
  def __init__(
      self,
      agent_name:str,
      **kwargs,
...
def build_agent(
  ...
  observation_summary = agent_components.observation.ObservationSummary(
      model=model,
      clock_now=clock.now,
      timeframe_delta_from=datetime.timedelta(hours=10),
  ...
  behaviour_questions = _make_about_component(agent_name, measurements, model, clock)
  ...
  entity_components = core_components + tuple(behaviour_questions) + tuple(question_components)
  components_of_agent = {
